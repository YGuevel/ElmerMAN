...
!------------------------------------------------------------------------------
   SUBROUTINE FlowSolverCorrection_init( Model,Solver,Timestep,TransientSimulation )
!------------------------------------------------------------------------------
     USE DefUtils
     IMPLICIT NONE
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
   ...
   END SUBROUTINE FlowSolverCorrection_init


!------------------------------------------------------------------------------
!> Solver for the Navier-Stokes equation in various different coordinate systems.
!------------------------------------------------------------------------------
   SUBROUTINE FlowSolverCorrection( Model,Solver,dt,TransientSimulation,LAMBDA0)
!------------------------------------------------------------------------------
    USE NavierStokes
...
!------------------------------------------------------------------------------
     CALL Info('FlowSolveCorrection','CORRECTION FOR the Navier-Stokes equations',Level=5)
     ...
     IF ( UseLocalCoords ) THEN
        ...
        SELECT CASE ( LocalCoords )
           CASE( 'cartesian 2d' )
              Coordinates = 1
              Model % DIMENSION = 2
              CALL Info( 'FlowSolveCorrection', 'Solver Coordinate System is cartesian 2d', LEVEL=31 )
           CASE( 'cartesian 3d' )
              Coordinates = 1
              Model % DIMENSION = 3
              CALL Info( 'FlowSolveCorrection', 'Solver Coordinate System is cartesian 3d', LEVEL=31 )
           CASE( 'axi symmetric' )
              Coordinates = 4
              Model % DIMENSION = 2
              CALL Info( 'FlowSolveCorrection', 'Solver Coordinate System is axi symmetric', LEVEL=31 )
           CASE( 'cylindric symmetric' )
              Coordinates = 3
              Model % DIMENSION = 3
              CALL Info( 'FlowSolveCorrection', 'Solver Coordinate System is cylindric symmetric', LEVEL=31 )
           CASE DEFAULT
              CALL Warn( 'FlowSolveCorrection', 'Solver coordinate system not recognised, using original' )
        END SELECT
     END IF
...
!------------------------------------------------------------------------------
     ...
     IF ( .NOT.AllocationsDone .OR. Solver % Mesh % Changed ) THEN
       ...
       IF ( istat /= 0 ) THEN
         CALL Fatal( 'FlowSolveCorrection','Memory allocation error, Aborting.' )
       END IF
...
!------------------------------------------------------------------------------
     ...
     IF(PseudoPressureExists) THEN
       ...
       CALL Info('FlowSolveCorrection',Message,Level=5)
     ...
     DO iter=1,NonlinearIter
       ...
       CALL Info( 'FlowSolveCorrection', ' ', Level=4 )
       CALL Info( 'FlowSolveCorrection', ' ', Level=4 )
       CALL Info( 'FlowSolveCorrection', '-------------------------------------', Level=4 )
       WRITE( Message, * ) 'NAVIER-STOKES ITERATION', iter 
       CALL Info( 'FlowSolveCorrection',Message, Level=4 )
       IF (NewtonLinearization) THEN
        WRITE( Message, * ) 'NEWTON ITERATION'
       ELSE 
        WRITE( Message, * ) 'PICARD ITERATION'
       ENDIF
       CALL Info( 'FlowSolveCorrection',Message, Level=4 )
       
       CALL Info( 'FlowSolveCorrection','-------------------------------------', Level=4 )
       CALL Info( 'FlowSolveCorrection', ' ', Level=4 )
       CALL Info( 'FlowSolveCorrection','Starting Assembly...', Level=4 )
...
!------------------------------------------------------------------------------
       ...
       CALL StartAdvanceOutput( 'FlowSolveCorrection', 'Assembly: ' )
       DO t = 1,GetNOFActive()
           ...
           IF( bf_id > 0 ) THEN
             ...
             IF ( GotIt ) THEN
               IF( Coordinates == Cartesian ) THEN
                 ...
                 Rotating = .TRUE.
               ELSE
                 CALL Fatal('FlowSolveCorrection','Rotating coordinate implemented only for cartesian coodinates')
               END IF
             ELSE
             ...
             DO i=1,n
               ...
               IF ( k > 0 ) THEN
                 IF ( Hydrostatic ) THEN
                   ...
                   IF ( Tdiff <= 0.0D0 ) THEN
                      CALL Warn( 'FlowSolveCorrection','Zero or negative density.' )
                   END IF
                 ELSE
...
!------------------------------------------------------------------------------
      ...
      CALL Info( 'FlowSolveCorrection', 'Assembly done', Level=4 )
...
!------------------------------------------------------------------------------
!       CALL DefaultDirichletBCsLAMBDA(LAMBDA0)
      CALL DefaultDirichletBCs()
      CALL Info( 'FlowSolveCorrection', 'Dirichlet LAMBDA0 conditions done', Level=4 )
...
!------------------------------------------------------------------------------
     ...
     IF ( l > 0 ) THEN
        ...
        CALL Info( 'FlowSolveCorrection', Message, Level=6 )
     END IF
!------------------------------------------------------------------------------
...
! ! ! ! ! ! !------------------------------------------------------------------------------------------
! ! ! ! ! !   SUBROUTINE DefaultDirichletBCsLAMBDA( LAMBDA0 ,USolver,Ux,UOffset,OffDiagonalMatrix,PiolaCurlTransform)
! ! ! ! ! ! !------------------------------------------------------------------------------------------
...
! ! ! ! ! !        IF ( istat /= 0 ) &
! ! ! ! ! !           CALL Fatal('DefUtils::DefaultDirichletBCsLAMBDA','Memory allocation failed.' )
...
! ! ! ! ! !      END IF
! ! ! ! ! ! 
! ! ! ! ! ! !      write (*,*) "DefaultDirichletBCsLAMBDA x % DOFs=",x % DOFs
! ! ! ! ! ! 
! ! ! ! ! ! 
! ! ! ! ! !      IF ( x % DOFs > 1 ) THEN
...
! ! ! ! ! !      END IF
! ! ! ! ! ! 
! ! ! ! ! !      CALL Info('DefUtils::DefaultDirichletBCsLAMBDA', &
...
! ! ! ! ! !      DO DOF=1,x % DOFs
! ! ! ! ! !         name = x % name
! ! ! ! ! ! !         write (*,*) "DefaultDirichletBCsLAMBDA x % name=",x % name
! ! ! ! ! !         
! ! ! ! ! !         IF ( x % DOFs > 1 ) name = ComponentName(name,DOF)
! ! ! ! ! ! !         write (*,*) "DefaultDirichletBCsLAMBDA name ComponentName=",TRIM(Name)
! ! ! ! ! !         
...
! ! ! ! ! ! !            WRITE(*,*) "isActivePElement name ",TRIM(Name)," ActiveBoundaryElement (,",i,")"
! ! ! ! ! ! 
! ! ! ! ! !            ptr => ListFind(BC, Name,Found )
! ! ! ! ! !            
! ! ! ! ! ! !            WRITE(*,*) "name ",TRIM(Name)," ActiveBoundaryElement (,",i,") - ptr % PROCEDURE/TYPE=",ptr % PROCEDURE," -",ptr % TYPE
! ! ! ! ! !            
! ! ! ! ! !            ConstantValue =  ptr % PROCEDURE == 0 .AND. &
! ! ! ! ! !              ptr % TYPE == LIST_TYPE_CONSTANT_SCALAR
...
! ! ! ! ! !               write (*,*) "LocalBcBDOFs BC Name=",Name," Work=",Work
! ! ! ! ! ! 
...
! ! ! ! ! !      END IF
! ! ! ! ! ! 
! ! ! ! ! !      CALL Info('DefUtils::DefaultDirichletBCsLAMBDA','Dirichlet boundary conditions set', Level=5)
! ! ! ! ! !      
! ! ! ! ! ! !------------------------------------------------------------------------------
! ! ! ! ! !   END SUBROUTINE DefaultDirichletBCsLAMBDA
! ! ! ! ! ! !------------------------------------------------------------------------------
! ! ! ! ! ! 
! ! ! ! ! !     SUBROUTINE TESTVALUELISTBC(Model)
! ! ! ! ! !        TYPE(Model_t)           :: Model          !< The current model structure
! ! ! ! ! !        
! ! ! ! ! !        TYPE(Mesh_t), POINTER   :: Mesh
! ! ! ! ! !        TYPE(Solver_t), POINTER :: Solver
! ! ! ! ! !     
! ! ! ! ! !        REAL(KIND=dp), POINTER  :: Work(:), WorkA(:,:,:) => NULL()
! ! ! ! ! !        INTEGER                 :: bndry_start, bndry_end, t, i, n, DOF,nlen
...
! ! ! ! ! !        Mesh   => Solver % Mesh  
! ! ! ! ! !        
! ! ! ! ! !        ALLOCATE( Indexes(Model % Mesh % MaxElementDOFs),Work(Model % Mesh % MaxElementDOFs) ) 
! ! ! ! ! !         
! ! ! ! ! ! ! DO DOF        
...
! ! ! ! ! !         nlen = Model % Solver % Variable % NameLen
! ! ! ! ! ! 
...
! ! ! ! ! ! !             IF ( Element % BoundaryInfo % Constraint /=  Model % BCs(i) % Tag ) CYCLE
...
! ! ! ! ! !        END DO
! ! ! ! ! !         
! ! ! ! ! !       END SUBROUTINE TESTVALUELISTBC
...
!------------------------------------------------------------------------------
  END SUBROUTINE FlowSolverCorrection
!------------------------------------------------------------------------------
...
! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !   END FUNCTION FlowInsideResidual
! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !------------------------------------------------------------------------------
END MODULE FlowSolveCorrection
!> \} IMNS
